...
public class WumpusKnowledgeBase extends KnowledgeBase {
    ...
    public static final String FACING_NORTH = PlayerPosition.Orientation.FACING_NORTH.getSymbol();
    public static final String FACING_SOUTH = PlayerPosition.Orientation.FACING_SOUTH.getSymbol();
    public static final String FACING_EAST = PlayerPosition.Orientation.FACING_EAST.getSymbol();
    public static final String FACING_WEST = PlayerPosition.Orientation.FACING_WEST.getSymbol();
    ...
    public WumpusKnowledgeBase(int caveXDim, int caveYDim, EntailmentChecker checker) {
        this(caveXDim, caveYDim, new PlayerPosition(1, 1, PlayerPosition.Orientation.FACING_NORTH), checker);
    }
     ...
     */
    public WumpusKnowledgeBase(int caveXDim, int caveYDim, PlayerPosition start, EntailmentChecker checker) {
        this.start = start;
    ...
    public PlayerPosition askCurrentPosition(int t) {
        int locX = -1, locY = -1;
        ...
        if (ask(newSymbol(FACING_NORTH, t)))
            current = new PlayerPosition(locX, locY, PlayerPosition.Orientation.FACING_NORTH);
        else if (ask(newSymbol(FACING_SOUTH, t)))
            current = new PlayerPosition(locX, locY, PlayerPosition.Orientation.FACING_SOUTH);
        else if (ask(newSymbol(FACING_EAST, t)))
            current = new PlayerPosition(locX, locY, PlayerPosition.Orientation.FACING_EAST);
        else if (ask(newSymbol(FACING_WEST, t)))
            current = new PlayerPosition(locX, locY, PlayerPosition.Orientation.FACING_WEST);
        else
...
//				}
     ...
     * @param t current time step.
     */
    public void tellTemporalPhysicsSentences(int t, PlayerPosition PlayerPosition) {
        if (t == 0) {
            // temporal rules at time zero
            tell(newSymbol(HAVE_ARROW, 0));
            tell(newSymbol(WUMPUS_ALIVE, 0));
        }
        tell(newSymbol(LOCATION, t, PlayerPosition.getX(), PlayerPosition.getY()));
        tell(newSymbol(PlayerPosition.getOrientation().getSymbol(), t));
        // Optimization to make questions about unvisited locations faster
        tell(newSymbol(LOCATION_VISITED, PlayerPosition.getX(), PlayerPosition.getY()));
        ...
        // and any square [x,y], we assert
        tell(new ComplexSentence(
                newSymbol(LOCATION, t, PlayerPosition.getX(), PlayerPosition.getY()),
                Connective.IMPLICATION,
                new ComplexSentence(newSymbol(PERCEPT_BREEZE, t), Connective.BICONDITIONAL,
                        newSymbol(BREEZE, PlayerPosition.getX(), PlayerPosition.getY()))));

        tell(new ComplexSentence(
                newSymbol(LOCATION, t, PlayerPosition.getX(), PlayerPosition.getY()),
                Connective.IMPLICATION,
                new ComplexSentence(newSymbol(PERCEPT_STENCH, t), Connective.BICONDITIONAL,
                        newSymbol(STENCH, PlayerPosition.getX(), PlayerPosition.getY()))));

        tellCommonTemporalPhysicsSentences(t);
        if (!disableNavSentences) {
            tellSuccessorStateLocationAxiom(t, PlayerPosition.getX(), PlayerPosition.getY());
            tellSuccessorStateOrientationAxioms(t);
        }
    }
